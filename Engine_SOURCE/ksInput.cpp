...
namespace ks
{
	...
	math::Vector3 Input::mMouseWorldPosition;;
	...
	void Input::Update()
	{
		if (GetFocus())
		{
			...
			Viewport mousepos = Viewport(viewport.TopLeftX, viewport.TopLeftY
			...
			mousepos.Unproject(mMousPosition, renderer::mainCamera->GetGpuProjectionMatrix()
				, renderer::mainCamera->GetGpuViewMatrix(), matrix, mMouseWorldPosition);

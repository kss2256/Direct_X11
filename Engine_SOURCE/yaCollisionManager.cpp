...
namespace ya
{
	...
	bool CollisionManager::Intersect(Collider2D* left, Collider2D* right)
	{
		...
		{
			//사각형 끼리 충돌
			if (right->GetType() == enums::eColliderType::Rect)
			{
			...
			//사각형 원 충돌 중복
			if (right->GetType() == enums::eColliderType::Circle)
			{
		...
		case ya::enums::eColliderType::Circle:
		{
			...
			{				
				...
				//float radiussum = leftsize.x / 2 + rightsize.x / 2;
				//radiussum *= radiussum;
				//float distance = rightpos.x - leftpos.x;
				//distance *= distance;	
				...
				float radiussum = leftsize.x / 2 + rightsize.x / 2;
				radiussum *= radiussum;
				Vector2 distance = Vector2(left->GetPosition().x - right->GetPosition().x, left->GetPosition().y - right->GetPosition().y);
				float lenght = distance.LengthSquared();
